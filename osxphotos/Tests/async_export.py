...
class Exporter:
   ...
   # Reads in Y-M-D as string and converts it to a datetime object
    ...
    async def processor(self, queue: Queue):
        consumables = []
        while True:
            if not queue.empty():
                for _ in range(os.cpu_count()*2):
                    consumables.append(create_task(self.consumer(queue)))
            await gather(*consumables)
    ...
    async def consumer(self, tasks: Queue):
        while True:
            if not tasks.empty():
                task, task_number = await tasks.get()
                start_time = time.perf_counter_ns()
                await task
                print(f"Finished task. total task execution time {(time.perf_counter_ns() - start_time)/10**9} Item {task_number}")
                tasks.task_done()
            else:
                return

    async def export(self, p, new_path, file_path, options, original_filename):
        try:
            # Run the export
            out = osxphotos.PhotoExporter(p).export(new_path, file_path, options)
            self.file_count += len(out.exported)
        except:
            # Unable to parse that file; log it to investigate later
            with open("log.txt", "a") as f:
                f.write(f"Had trouble writing {original_filename} at {str(datetime.datetime.now())}\n")           
    ...
    async def main(self) -> None:
        ...
        for i, p in enumerate(tqdm(self.photos)):
            ...
            if check1 not in check2:
                ...
                new_file = str_dt + "." + ext
                await self.tasks.put((self.export(p, new_path, new_file, fobj, original_filename), i))
            
            # Update the window after each photo
            #self.update_background(i, check_path.split("/")[-1])
        start_time = time.perf_counter_ns()
        print(f"Processing {self.tasks.qsize()} export tasks at {time.ctime()}")
        
        await self.processor(self.tasks)
        print(f"Total execution time: {(time.perf_counter_ns() - start_time)/10**9}")
...
async def main_loop():
    ...
    obj = Exporter()
    obj.dates("2022-10-21", "2030-12-22")
    await obj.main()
    obj.root.destroy()
    obj.root.mainloop()
